...
import environment_cent.Constant
...
def energy_by_speed(speed: float) -> float:
    ...
    return power_by_speed(speed) * environment_cent.Constant.time_slice
...
class UAV:
    ...
    def move_by_radian_rate(self, radian: float, rate: float):
        ...
        if not 0 <= rate <= 1:
            ...
            return False
        # 更新位置
        self.position.move_by_radian(radian, rate * self.speed_limit * environment_cent.Constant.time_slice)
