...
class DDQN(Agent):
    ...
    def interact(self):
        ...
        super(DDQN, self)._take_one_step()
    ...
    def train_one_step(self):
        ...
        # 将next states传入当前actor，得到next_action
        choose_next_action = self.actor(next_states_var).detach()
        next_state_action_index = th.argmax(choose_next_action, 1).view(-1, 1)
        # 计算Q(s_{t+1})向量，并从中选取next_action对应的Q(s_{t+1},a_{t+1})
        next_state_action_values = self.actor_target(next_states_var).detach()
        next_q = next_state_action_values.gather(1, next_state_action_index).view(-1, 1)
        ...
        if self.n_steps % self.target_update_steps == 0 and self.n_steps > 0:
            super(DDQN, self)._soft_update_target(self.actor_target, self.actor)
    ...
    def train(self):
        ...
        filename = directory + '/' + "DDQN_model.pth"
        th.save(self.actor.state_dict(), filename)

    def test(self):
        filename = "Pretrained_model/DDQN_model.pth"
        try:
